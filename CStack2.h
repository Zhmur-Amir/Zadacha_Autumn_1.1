...
class CStack2
{
    int n,size1; int *s;;
public:
    CStack2(){SetZero(); s=new int[size1=L];}
    void SetZero(){s=NULL; n=size1=0;}
    CStack2(const CStack2&b){CopyOnly(b);}
    void CopyOnly(const CStack2&b){n=b.n;  s=new int[size1=b.size1]; for(int i=0;i<n;i++){s[i]=b.s[i];}}
    void Clean(){delete[] s; SetZero();}
    ~CStack2(){Clean();}
    CStack2 &operator=(const CStack2&b){if(this!=&b){Clean(); CopyOnly(b);}return *this;}
